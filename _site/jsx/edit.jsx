...
var Signin = React.createClass({
  render: function() {
    return (
      <div>
        <div className="panel panel-primary signin">
          <div className="panel-heading">
            <h3 className="panel-title">Personal Access Token</h3>
          </div>
          ...
          <form className="form-inline" role="form">
            ...
            <p>Sign into Github > Click Account Settings > Select Applications > Personal access tokens > <strong>Generate new token</strong> </p>
          </div>
        </div>
      </div>
    )
...
var Nav = React.createClass({
  render: function() {
    return (
      <div>
        <nav className="topNav bg-primary">
          {this.props.home}
          {this.props.all}
          {this.props.newPost}
          <span className="title">
          <label for="postTitle">Path: </label>
...
var MarkdownEditor = React.createClass({
  ...
  render: function() {
    ...
    if (this.state.sha === '') {
      var commit = <button style={inputStyle} onClick={this.handleCommit} className='btn btn-primary pull-right btn-success'>Add Post</button>
    } else {
      var commit =  <button style={inputStyle} className="btn btn-primary pull-right btn-info" onClick={this.handleCommit}>Commit</button>

    }
    var home = <button className="btn btn-default pull-left" onClick = {this.handleHome} >Home</button>
    var auth = <input type="text" className="form-control auth" value={github} onChange={this.handleAuth} />
    var submit =  <button type="submit" className="btn btn-default submit" onClick={this.handleSignin}>Submit</button>
    var edit = <button style={outputStyle} onClick={this.handleEdit} className='btn btn-primary btn-warning pull-right'>Edit Post</button>
    var all = <button style={inputStyle} onClick={this.handleAll} className='btn btn-primary pull-left'>All Post</button>
    var newPost = <button style={outputStyle} onClick={this.handleNew} className='btn btn-primary btn-info pull-left'>New Post</button>
    var deletebtn =  <button className="btn btn-danger pull-right" onClick={this.handleDelete}>Delete Post</button>
    var postTitle = <input type="text" id="postTitle" className="bg-primary" onChange = {this.handlePathInput} value={this.state.path} />
    ...
    return (
      <div className="">
        <Nav home={home} edit={edit} all={all} newPost={newPost} deletebtn={deletebtn} postTitle={postTitle} commit={commit} />
        <div ref="input" style={inputStyle} className="input">
        <textarea onChange={this.handleLayout} className="layout" value={this.state.layout} />
          <textarea
            className="markdown"
        ...
        <nav className="bottomNav bg-primary" >
          <p className="" >--- Logged in as: {login} --- Requests Remaining: {this.state.remaining} --- <span className="pull-right">--- Sha: {this.state.sha} ---</span> </p>
        </nav>
