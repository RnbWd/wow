...
var MarkdownEditor = React.createClass({displayName: 'MarkdownEditor',
  ...
  render: function() {
    ...
    if (this.state.sha === '') {
      var commit = React.DOM.button( {style:inputStyle, onClick:this.handleCommit, className:"btn btn-primary pull-right"}, "Add Post")
    } else {
    ...
    $.ajax({
        type: "PUT",
         ...
         success: function(response) {
          console.log(response);
          if (self.state.sha == '') {
            $('#contentNav').prepend("<a class='list-group-item' onClick='editPost(event, "+self.state.path+", "+self.state.title+")' href='#'>"+self.state.title+"</a>");
          }
