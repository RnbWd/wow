...
var MarkdownEditor = React.createClass({displayName: 'MarkdownEditor',
  getInitialState: function() {
    return {value: 'Type some *markdown* here!', github: auth.first().get('token'), path: '', signin: '', login: '', sha: '', limit: '', remaining: '', layout: '', title: ''};
  },
  componentDidMount: function() {
     if (this.state.github != '') {
      this.handleSignin();
      this.getPost(array[0].path);
     }
  },
...
var Signin = React.createClass({displayName: 'Signin',
  ...
  componentDidMount: function() {
    var self = this;
     ...
     this.forceUpdate();
     var tween = new TweenLite.to(self.props.style, 2, {width:200, delay: 2, onComplete: function() {console.log(self.props.style)}, onUpdate: function(){self.forceUpdate()}});
  },
  render: function() {
    return (
      React.DOM.div( {className:"container-fluid brdr github"}, 
        React.DOM.div(null, 
        React.DOM.div( {style:this.props.style}
        ),
