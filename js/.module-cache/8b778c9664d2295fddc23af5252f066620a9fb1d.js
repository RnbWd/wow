    ...
    if (event) {
      ...
      var doIt = function() {
       ...
       // window.requestAnimationFrame(self.update);
        var bannerTween = new TweenMax.to(weird, 60, {rotation: (total),rotationY: (total), transformOrigin:"50% 50% -200px", transformPerspective:400, force3D: true, useFrames: true});
        var bannerTween2 = new TweenMax.to(confuse, 60, {width: (total/2), transformOrigin:"50% 50% -200px", transformPerspective:400, force3D: true, useFrames: true});
          var bannerTween3 = new TweenMax.to(comp, 60, {height: (total/4), rotationX: (-total), transformOrigin:"50% 50% -200px", transformPerspective:400, force3D: true, useFrames: true});
  ...
  render: function() {
    ...
    return (
      React.DOM.div(  {className:"weird",  style:{height: window.innerHeight}, onWheel:this.handleScroll, onTouchMove:this.handleTouch}, 
        React.DOM.div( {className:"shibe", style:{position: 'fixed', zIndex: 60}, ref:"weird"} 
