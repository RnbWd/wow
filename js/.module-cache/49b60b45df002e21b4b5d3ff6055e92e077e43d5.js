    ...
    if (event) {
      ...
      var doIt = function() {
       ...
       // window.requestAnimationFrame(self.update);
        ...
        var bannerTween2 = new TweenMax.to(confuse, 60, {height: (total/2), transformOrigin:"50% 50% -200px", transformPerspective:400, force3D: true, useFrames: true});
          var bannerTween3 = new TweenMax.to(comp, 60, {height: (total/4), rotation: (-total), transformOrigin:"50% 50% -200px", transformPerspective:400, force3D: true, useFrames: true});
  ...
  render: function() {
    ...
    var style = {
                 ...
                 WebkitClipPath: 'polygon(\
                  50% 0, 80% 0, 50% '+(middleH+25)+'px,\
                  50% 90%, 100% 20px, 100% 50%,\
                  75% 100%, 25% 100%, 0 50%, 0 0,\
                  50% 90%, 50% '+(middleH+25)+'px, 20% 0\
                )'
                
               };

    return (
      React.DOM.div(  {className:"weird",  onWheel:this.handleScroll, onTouchMove:this.handleTouch}, 
        ...
        React.DOM.img( {style:{height: 0, position: 'fixed', left: '30%', top: '15%',zIndex: 20, borderRadius: '50%'}, ref:"comp", src:"images/cs.jpg"} )
      )
    )
  }
});
