...
var MarkdownEditor = React.createClass({displayName: 'MarkdownEditor',
  ...
  render: function() {
    ...
    if (this.state.sha === '') {
      var commit = React.DOM.button( {style:inputStyle, onClick:this.handleAdd, className:"btn btn-primary pull-right"}, "Add Post")
    } else {
      var commit =  React.DOM.button( {style:inputStyle, className:"btn btn-primary pull-right", onClick:this.handleCommit}, "Commit")

    }
    var auth = React.DOM.input( {type:"text", className:"form-control auth", value:github, onChange:this.handleAuth} )
    var pathInput = React.DOM.input( {type:"text", className:"form-control", value:path, onChange:this.handlePathInput} )
    ...
    var deletebtn =  React.DOM.button( {style:inputStyle, className:"btn btn-primary pull-right", onClick:this.handleDelete}, "Delete")
    ...
    return (
      React.DOM.div( {className:""}, 
        ...
        newPost,
          ...
          React.DOM.label( {for:"postTitle"}, "Title: " ),
          React.DOM.input( {type:"text", id:"postTitle", className:"bg-primary", onChange:  this.handleTitle, value:this.state.title} )
          ),
          commit,
          deletebtn
          ),
        ...
        React.DOM.nav( {className:"bottomNav bg-primary"} , 
          React.DOM.p( {className:""} , "Logged in as: ", login, " Limit: ", this.state.limit, " Remaining: ", this.state.remaining,  "  Path: ", pathInput)
        )
    ...
    $.ajax({
        type: "GET",
         ...
         success: function(response) {
          ...
          if (split.length < 2) {
            self.setState({value: auto, path0: response.path, path1: response.path, sha: response.sha});
          } else {
            self.setState({value: split[2].trim(), layout: split[1].trim(), path0: response.path, path1: response.path, sha: response.sha});
          }
          /*var data = self.state.path.split("/")[1].split("-");
          console.log(data);
  ...
  handleAdd: function() {
    $.ajax({
        type: "PUT",
         url: "https://api.github.com/repos/RnbWd/portfolio/contents/_posts/"+this.state.path0,
         headers: {
          Authorization: "token "+this.state.github
         },
         datatype: "jsonp",
         data: JSON.stringify({"path": "/_posts/"+this.state.path1, "message": "hello", "content": btoa(this.state.value)}),
         success: function(response) {
          console.log(response);
         },
         error: function(response) {
  ...
  handleCommit: function() {
    ...
    $.ajax({
        type: "PUT",
         url: "https://api.github.com/repos/RnbWd/portfolio/contents/"+this.state.path0,
         headers: {
          Authorization: "token "+this.state.github
         },
         datatype: "jsonp",
         data: JSON.stringify({"path": this.state.path1, "message": "hello", "content": btoa("---\n"+this.state.layout+"\n---\n"+this.state.value), "sha": this.state.sha}),
         success: function(response) {
          console.log(response);
  ...
  handlePathInput: function(event) {
   this.setState({path1: event.target.value});
  },
